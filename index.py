...
class TokenManager:
    ...
    def save_tokens(user_token: str, refresh_token: str) -> None:
        ...
        try:
            ...
            # 使用 pyminizip 压缩并加密
            os.makedirs(os.path.dirname(TOKEN_ZIP_PATH), exist_ok=True)
            pyminizip.compress(tmp_path, None, TOKEN_ZIP_PATH, PASSWORD, 5)

            # 删除临时文件
            os.remove(tmp_path)
            logger.info(f"Token已加密保存到: {TOKEN_ZIP_PATH}")
    ...
    def load_tokens() -> Optional[Dict[str, str]]:
        """从加密zip文件加载token"""
        try:
            if not os.path.exists(TOKEN_ZIP_PATH):
                logger.info("Token文件不存在，将使用账号密码登录")
                return None

            # 使用临时目录解压
            with tempfile.TemporaryDirectory() as tmp_dir:
                pyminizip.uncompress(TOKEN_ZIP_PATH, PASSWORD, tmp_dir, 0)
                token_file_path = os.path.join(tmp_dir, "tokens.json")
                with open(token_file_path, "r", encoding="utf-8") as f:
                    token_data = json.load(f)

            logger.info(f"从加密文件加载token成功，保存时间: {token_data.get('saved_at', '未知')}")
            return token_data
        except Exception as e:
...
if __name__ == "__main__":
    ...
    logging.shutdown()
    time.sleep(0.5)
    os._exit(0)  # 确保彻底退出import logging
import os
import json
import smtplib
import zipfile
